...
const formItemClasses = computed(() => [
  ...
  ns.is(
    'error',
    validateState.value === 'error' && !formContext?.isDisabledError
  ),
  ...
  ns.is(
    ...
    (isRequired.value || props.required) &&
      formContext?.labelPosition === 'right'
  ),
  ns.is(
    'required-right',
    (isRequired.value || props.required) &&
...
const fieldValue = computed(() => {
  ...
  return Array.isArray(model) ? model : getProp(model, props.prop).value
})
  ...
  return (
    rules
      .filter((rule) => {
        ...
        if (
          formContext?.excludeComponentTriggerCallbackRule &&
